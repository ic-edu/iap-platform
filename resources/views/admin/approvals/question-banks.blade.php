...
@section('content')
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Review pending question bank submissions submitted by Teachers for Super Admin authorization.</p>
        </div>
        <div>
                ...
                @forelse ($pendingQuestionBanks as $bank)
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-950/40 transition-colors">
                        <td class="p-4 font-semibold text-slate-900 dark:text-white">
                            {{ $bank->title }}
                            <div class="text-xs text-slate-400 dark:text-slate-500 font-normal">Category: {{ $bank->category?->name ?? 'General' }} | Items: {{ $bank->questions->count() }}</div>
                        </td>
                        ...
                        <td class="p-4 text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-12 text-center">
                            ...
                            <div class="text-sm font-bold text-slate-900 dark:text-white mb-1">No Pending Question Banks</div>
                            <div class="text-xs text-slate-400 dark:text-slate-500">All submitted question banks have been reviewed and authorized.</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
