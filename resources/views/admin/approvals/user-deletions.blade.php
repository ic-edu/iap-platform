...
@section('content')
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Review pending user account deletion requests submitted by Administrators.</p>
        </div>
        <div>
            <span class="px-3 py-1.5 text-xs font-bold rounded-full bg-rose-500/10 text-rose-700 dark:text-rose-300 border border-rose-500/20">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse ($userDeletionRequests as $delReq)
                    ...
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-950/40 transition-colors">
                        <td class="p-4">
                            <div class="font-semibold text-slate-900 dark:text-white">{{ $target?->name ?? 'User Account' }}</div>
                            <div class="text-xs font-mono text-indigo-600 dark:text-indigo-400">{{ $target?->email ?? 'Deleted User' }}</div>
                        </td>
                        <td class="p-4">
                        ...
                        <td class="p-4 text-xs text-slate-700 dark:text-slate-300">
                            ...
                            <div class="text-slate-400 dark:text-slate-500 font-mono">{{ $delReq->requester?->email }}</div>
                        </td>
                        <td class="p-4 text-xs text-slate-700 dark:text-slate-300 max-w-xs">
                            <span class="italic bg-slate-50 dark:bg-slate-950 p-2 rounded border border-slate-200 dark:border-slate-800 block text-slate-700 dark:text-slate-300">{{ $delReq->reason }}</span>
                        </td>
                        <td class="p-4 text-xs text-slate-500 dark:text-slate-400">{{ $delReq->created_at?->format('Y-m-d H:i') }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="p-12 text-center">
                            ...
                            <div class="text-sm font-bold text-slate-900 dark:text-white mb-1">No Pending User Deletion Requests</div>
                            <div class="text-xs text-slate-400 dark:text-slate-500">All enterprise user deletion workflows are up to date.</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
