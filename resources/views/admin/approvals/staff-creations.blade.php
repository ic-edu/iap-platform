...
@section('content')
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Review pending internal staff user creations (Teachers, Finance, Admins) submitted by Administrators.</p>
        </div>
        <div>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse ($userCreationRequests as $createReq)
                    ...
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-950/40 transition-colors">
                        <td class="p-4">
                            <div class="font-semibold text-slate-900 dark:text-white">{{ $target?->name ?? 'Staff Account' }}</div>
                            <div class="text-xs font-mono text-indigo-600 dark:text-indigo-400">{{ $target?->email ?? 'N/A' }}</div>
                        </td>
                        <td class="p-4">
                        ...
                        <td class="p-4 text-xs text-slate-700 dark:text-slate-300">
                            ...
                            <div class="text-slate-400 dark:text-slate-500 font-mono">{{ $createReq->requester?->email }}</div>
                        </td>
                        <td class="p-4 text-xs text-slate-500 dark:text-slate-400">{{ $createReq->created_at?->format('Y-m-d H:i') }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-12 text-center">
                            ...
                            <div class="text-sm font-bold text-slate-900 dark:text-white mb-1">No Pending Staff Creation Requests</div>
                            <div class="text-xs text-slate-400 dark:text-slate-500">All staff user creation workflows are up to date.</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
