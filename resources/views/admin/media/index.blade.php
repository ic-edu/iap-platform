<x-admin-layout>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-1">Upload, preview, and manage audio tracks, image diagrams, passages, and PDF attachments.</p>
        </div>
        <div>
            <label class="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-lg shadow transition-colors cursor-pointer inline-flex items-center gap-2">
                ...
                <input type="file" class="hidden" accept="image/*,audio/*,.pdf" onchange="alert('Media file uploaded successfully!')">
            </label>
        </div>
    </div>
        ...
        @foreach ($mediaItems as $item)
            <div class="p-4 bg-slate-900 border border-slate-800 rounded-xl flex flex-col justify-between">
                ...
                <div class="mt-4 pt-3 border-t border-slate-800/80 flex justify-between items-center text-xs">
                    <span class="text-slate-400 font-mono">{{ $item['id'] }}</span>
                    <button type="button" class="text-rose-400 hover:underline">Delete</button>
                </div>
            </div>
        @endforeach
    </div>
