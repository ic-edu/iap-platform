<?php

namespace Tests\Feature;

use App\Models\User;
use App\Modules\Assessment\Models\Attempt;
use App\Modules\Assessment\Models\CandidateTestAssignment;
use App\Modules\Certificate\Models\Certificate;
use App\Modules\Commerce\Domain\Enums\InvoiceStatus;
use App\Modules\Commerce\Domain\Enums\OrderStatus;
use App\Modules\Commerce\Domain\Enums\PaymentStatus;
use App\Modules\Commerce\Domain\Models\Invoice;
use App\Modules\Commerce\Domain\Models\Order;
use App\Modules\Commerce\Domain\Models\OrderItem;
use App\Modules\Commerce\Domain\Models\Payment;
use App\Modules\Commerce\Domain\Models\Product;
use Database\Seeders\RolesAndPermissionsSeeder;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\DB;
use Tests\TestCase;

class FinanceOperationalReportingTest extends TestCase
{
    use RefreshDatabase;

    protected User $financeUser;
    protected User $studentUser;
    protected User $adminUser;
    protected User $superAdminUser;

    protected Product $productToeic;
    protected Product $productToefl;

    protected Payment $uatPayment;
    protected Payment $legacyPayment;
    protected Payment $confirmedPayment;
    protected Payment $failedPayment;

    protected function setUp(): void
    {
        parent::setUp();

        $this->seed(RolesAndPermissionsSeeder::class);

        $this->financeUser = User::factory()->create([
            'email' => 'finance_test@icedu.org',
            'name'  => 'Finance Officer',
        ]);
        $this->financeUser->assignRole('finance');

        $this->studentUser = User::factory()->create([
            'email' => 'student@icedu.org',
            'name'  => 'Candidate Student',
        ]);
        $this->studentUser->assignRole('student');

        $this->adminUser = User::factory()->create([
            'email' => 'admin@icedu.org',
            'name'  => 'Operational Admin',
        ]);
        $this->adminUser->assignRole('admin');

        $this->superAdminUser = User::factory()->create([
            'email' => 'superadmin@icedu.org',
            'name'  => 'Super Admin',
        ]);
        $this->superAdminUser->assignRole('super-admin');

        // Products
        $this->productToeic = Product::create([
            'title'             => 'TOEIC Mock Test Package',
            'slug'              => 'toeic-mock-test-package',
            'product_type'      => 'assessment',
            'price'             => 750000,
            'is_active'         => true,
        ]);

        $this->productToefl = Product::create([
            'title'             => 'TOEFL iBT Standard Package',
            'slug'              => 'toefl-ibt-standard-package',
            'product_type'      => 'assessment',
            'price'             => 900000,
            'is_active'         => true,
        ]);

        // UAT Payment: PAY-20260827-VZDM (Pending)
        $orderUat = Order::create([
            'user_id'      => $this->studentUser->id,
            'order_number' => 'ORD-20260827-YIT4',
            'status'       => OrderStatus::Pending,
            'subtotal'     => 750000,
            'discount'     => 0,
            'tax'          => 82500,
            'grand_total'  => 832500,
        ]);
        DB::table('orders')->where('id', $orderUat->id)->update(['created_at' => '2026-08-27 10:00:00']);

        OrderItem::create([
            'order_id'   => $orderUat->id,
            'product_id' => $this->productToeic->id,
            'quantity'   => 1,
            'price'      => 750000,
            'total'      => 750000,
        ]);

        $invoiceUat = Invoice::create([
            'order_id'       => $orderUat->id,
            'user_id'        => $this->studentUser->id,
            'invoice_number' => 'INV-20260827-ZMJY',
            'status'         => InvoiceStatus::Unpaid,
            'amount'         => 832500,
            'due_date'       => Carbon::parse('2026-08-29 10:00:00'),
        ]);
        DB::table('invoices')->where('id', $invoiceUat->id)->update(['created_at' => '2026-08-27 10:00:00']);

        $this->uatPayment = Payment::create([
            'invoice_id'          => $invoiceUat->id,
            'user_id'             => $this->studentUser->id,
            'reference_number'    => 'PAY-20260827-VZDM',
            'payment_gateway'     => 'manual_transfer',
            'status'              => PaymentStatus::Pending,
            'amount'              => 832500,
            'proof_path'          => 'payment-proofs/sample.png',
            'proof_original_name' => 'receipt.png',
            'proof_uploaded_at'   => Carbon::parse('2026-08-27 10:30:00'),
        ]);
        DB::table('payments')->where('id', $this->uatPayment->id)->update(['created_at' => '2026-08-27 10:00:00']);

        // Confirmed Payment (TOEFL)
        $orderConfirmed = Order::create([
            'user_id'      => $this->studentUser->id,
            'order_number' => 'ORD-20260820-AAAA',
            'status'       => OrderStatus::Completed,
            'subtotal'     => 900000,
            'discount'     => 50000,
            'tax'          => 93500,
            'grand_total'  => 943500,
        ]);
        DB::table('orders')->where('id', $orderConfirmed->id)->update(['created_at' => '2026-08-20 14:00:00']);

        OrderItem::create([
            'order_id'   => $orderConfirmed->id,
            'product_id' => $this->productToefl->id,
            'quantity'   => 1,
            'price'      => 900000,
            'total'      => 900000,
        ]);

        $invoiceConfirmed = Invoice::create([
            'order_id'       => $orderConfirmed->id,
            'user_id'        => $this->studentUser->id,
            'invoice_number' => 'INV-20260820-CONF',
            'status'         => InvoiceStatus::Paid,
            'amount'         => 943500,
            'paid_at'        => Carbon::parse('2026-08-20 15:00:00'),
        ]);
        DB::table('invoices')->where('id', $invoiceConfirmed->id)->update(['created_at' => '2026-08-20 14:00:00']);

        $this->confirmedPayment = Payment::create([
            'invoice_id'          => $invoiceConfirmed->id,
            'user_id'             => $this->studentUser->id,
            'reference_number'    => 'PAY-20260820-CONF',
            'payment_gateway'     => 'manual_transfer',
            'transaction_id'      => 'TXN-BCA-889911',
            'status'              => PaymentStatus::Success,
            'amount'              => 943500,
            'confirmed_at'        => Carbon::parse('2026-08-20 15:00:00'),
        ]);
        DB::table('payments')->where('id', $this->confirmedPayment->id)->update(['created_at' => '2026-08-20 14:00:00']);

        // Cancelled Payment
        $orderFailed = Order::create([
            'user_id'      => $this->studentUser->id,
            'order_number' => 'ORD-20260815-FAIL',
            'status'       => OrderStatus::Cancelled,
            'subtotal'     => 750000,
            'discount'     => 0,
            'tax'          => 82500,
            'grand_total'  => 832500,
        ]);
        DB::table('orders')->where('id', $orderFailed->id)->update(['created_at' => '2026-08-15 09:00:00']);

        OrderItem::create([
            'order_id'   => $orderFailed->id,
            'product_id' => $this->productToeic->id,
            'quantity'   => 1,
            'price'      => 750000,
            'total'      => 750000,
        ]);

        $invoiceFailed = Invoice::create([
            'order_id'       => $orderFailed->id,
            'user_id'        => $this->studentUser->id,
            'invoice_number' => 'INV-20260815-FAIL',
            'status'         => InvoiceStatus::Cancelled,
            'amount'         => 832500,
        ]);
        DB::table('invoices')->where('id', $invoiceFailed->id)->update(['created_at' => '2026-08-15 09:00:00']);

        $this->failedPayment = Payment::create([
            'invoice_id'          => $invoiceFailed->id,
            'user_id'             => $this->studentUser->id,
            'reference_number'    => 'PAY-20260815-FAIL',
            'payment_gateway'     => 'manual_transfer',
            'status'              => PaymentStatus::Failed,
            'amount'              => 832500,
            'proof_notes'         => 'Rejection reason: Invalid slip amount',
        ]);
        DB::table('payments')->where('id', $this->failedPayment->id)->update(['created_at' => '2026-08-15 09:00:00']);

        // Legacy Orphan (No Invoice / Order hierarchy)
        $this->legacyPayment = Payment::create([
            'invoice_id'       => null,
            'user_id'          => $this->studentUser->id,
            'reference_number' => 'INV-20260726-0001',
            'payment_gateway'  => 'manual_transfer',
            'status'           => PaymentStatus::Success,
            'amount'           => 750000,
        ]);
        DB::table('payments')->where('id', $this->legacyPayment->id)->update(['created_at' => '2026-07-26 10:00:00']);
    }

    public function test_01_finance_can_access_payment_and_invoice_reports(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $response->assertStatus(200);
        $response->assertSee('Payment &amp; Invoice Reports', false);
    }

    public function test_02_candidate_cannot_access_finance_reports(): void
    {
        $response = $this->actingAs($this->studentUser)->get(route('finance.payments.index'));
        $response->assertStatus(403);
    }

    public function test_03_pending_filter_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'pending']));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260815-FAIL');
    }

    public function test_04_confirmed_filter_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'success']));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260815-FAIL');
    }

    public function test_05_cancelled_rejected_filter_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'failed']));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260815-FAIL');
        $response->assertDontSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260820-CONF');
    }

    public function test_06_all_transactions_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'all']));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260827-VZDM');
        $response->assertSee('PAY-20260820-CONF');
        $response->assertSee('PAY-20260815-FAIL');
    }

    public function test_07_start_date_filter_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'start_date' => '2026-08-25',
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260815-FAIL');
    }

    public function test_08_end_date_filter_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'end_date' => '2026-08-18',
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260815-FAIL');
        $response->assertDontSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260820-CONF');
    }

    public function test_09_date_range_filter_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'start_date' => '2026-08-19',
            'end_date'   => '2026-08-22',
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260815-FAIL');
    }

    public function test_10_invalid_date_range_is_handled_safely(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'start_date' => 'invalid-date-string',
            'end_date'   => 'not-a-date',
        ]));
        $response->assertStatus(200);
        $response->assertSee('Payment &amp; Invoice Reports', false);
    }

    public function test_11_search_by_payment_reference_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'search' => 'PAY-20260827-VZDM',
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260820-CONF');
    }

    public function test_12_search_by_invoice_reference_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'search' => 'INV-20260820-CONF',
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260827-VZDM');
    }

    public function test_13_search_by_candidate_name_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'search' => 'Candidate Student',
        ]));
        $response->assertStatus(200);
        $response->assertSee('Candidate Student');
    }

    public function test_14_search_by_candidate_email_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'search' => 'student@icedu.org',
        ]));
        $response->assertStatus(200);
        $response->assertSee('student@icedu.org');
    }

    public function test_15_search_by_transaction_id_works_when_present(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'search' => 'TXN-BCA-889911',
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260827-VZDM');
    }

    public function test_16_product_package_filter_works(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'product_id' => $this->productToefl->id,
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260827-VZDM');
    }

    public function test_17_combined_filters_work(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'status'     => 'pending',
            'start_date' => '2026-08-26',
            'end_date'   => '2026-08-28',
            'search'     => 'VZDM',
            'product_id' => $this->productToeic->id,
        ]));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260827-VZDM');
        $response->assertDontSee('PAY-20260820-CONF');
        $response->assertDontSee('PAY-20260815-FAIL');
    }

    public function test_18_clear_filters_resets_the_query(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $response->assertStatus(200);
        $response->assertSee('PAY-20260827-VZDM');
        $response->assertSee('PAY-20260820-CONF');
        $response->assertSee('PAY-20260815-FAIL');
    }

    public function test_19_summary_transaction_count_is_accurate(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'all']));
        $response->assertStatus(200);
        $response->assertSee('Filtered Count');
    }

    public function test_20_pending_amount_is_accurate(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'all']));
        $response->assertStatus(200);
        $response->assertSee('IDR 832,500');
    }

    public function test_21_confirmed_amount_is_accurate(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'all']));
        $response->assertStatus(200);
        $response->assertSee('IDR 943,500');
    }

    public function test_22_cancelled_amount_is_accurate(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'all']));
        $response->assertStatus(200);
        $response->assertSee('IDR 832,500');
    }

    public function test_23_tax_ppn_values_use_persisted_transaction_data(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'all']));
        $response->assertStatus(200);
        $response->assertSee('Tax / PPN (11%)');
        $response->assertSee('Tax: +82,500');
    }

    public function test_24_csv_export_works_for_finance(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.export.csv'));
        $response->assertStatus(200);
        $this->assertEquals('text/csv; charset=UTF-8', $response->headers->get('Content-Type'));
    }

    public function test_25_csv_export_respects_filters(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.export.csv', [
            'status' => 'pending',
        ]));

        ob_start();
        $response->sendContent();
        $csvContent = ob_get_clean();

        $this->assertStringContainsString('PAY-20260827-VZDM', $csvContent);
        $this->assertStringNotContainsString('PAY-20260820-CONF', $csvContent);
    }

    public function test_26_csv_export_contains_required_columns(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.export.csv'));

        ob_start();
        $response->sendContent();
        $csvContent = ob_get_clean();

        $this->assertStringContainsString('Payment Reference', $csvContent);
        $this->assertStringContainsString('Payment Status', $csvContent);
        $this->assertStringContainsString('Payment Date', $csvContent);
        $this->assertStringContainsString('Candidate Name', $csvContent);
        $this->assertStringContainsString('Candidate Email', $csvContent);
        $this->assertStringContainsString('Product / Package', $csvContent);
        $this->assertStringContainsString('Order Reference', $csvContent);
        $this->assertStringContainsString('Invoice Reference', $csvContent);
        $this->assertStringContainsString('Base Amount (IDR)', $csvContent);
        $this->assertStringContainsString('Tax / PPN (IDR)', $csvContent);
        $this->assertStringContainsString('Grand Total (IDR)', $csvContent);
    }

    public function test_27_xlsx_export_works_for_finance(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.export.xlsx'));
        $response->assertStatus(200);
        $this->assertEquals('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', $response->headers->get('Content-Type'));
    }

    public function test_28_xlsx_export_respects_filters(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.export.xlsx', [
            'status' => 'pending',
        ]));
        $response->assertStatus(200);
    }

    public function test_29_unauthorized_users_cannot_export(): void
    {
        $responseCsv = $this->actingAs($this->studentUser)->get(route('finance.payments.export.csv'));
        $responseCsv->assertStatus(403);

        $responseXlsx = $this->actingAs($this->studentUser)->get(route('finance.payments.export.xlsx'));
        $responseXlsx->assertStatus(403);
    }

    public function test_30_pagination_preserves_filters(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'status' => 'pending',
            'search' => 'student',
        ]));
        $response->assertStatus(200);
        $response->assertSee('status=pending');
        $response->assertSee('search=student');
    }

    public function test_31_export_includes_full_filtered_dataset_not_current_page_only(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.export.csv', [
            'status' => 'all',
            'page'   => 1,
        ]));

        ob_start();
        $response->sendContent();
        $csvContent = ob_get_clean();

        $this->assertStringContainsString('PAY-20260827-VZDM', $csvContent);
        $this->assertStringContainsString('PAY-20260820-CONF', $csvContent);
        $this->assertStringContainsString('PAY-20260815-FAIL', $csvContent);
    }

    public function test_32_legacy_orphan_inv_20260726_0001_remains_excluded(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index', ['status' => 'all']));
        $response->assertStatus(200);
        $response->assertDontSee('INV-20260726-0001');
    }

    public function test_33_legacy_orphan_remains_in_database(): void
    {
        $this->assertDatabaseHas('payments', [
            'id'               => $this->legacyPayment->id,
            'reference_number' => 'INV-20260726-0001',
        ]);
    }

    public function test_34_pay_20260827_vzdm_remains_pending(): void
    {
        $this->assertEquals(PaymentStatus::Pending, $this->uatPayment->fresh()->status);
    }

    public function test_35_pay_20260827_vzdm_remains_idr_832500(): void
    {
        $this->assertEquals(832500, $this->uatPayment->fresh()->amount);
    }

    public function test_36_no_payment_state_mutation_during_reporting(): void
    {
        $initialStatuses = Payment::pluck('status', 'id')->toArray();

        $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $this->actingAs($this->financeUser)->get(route('finance.payments.export.csv'));
        $this->actingAs($this->financeUser)->get(route('finance.payments.export.xlsx'));

        $postStatuses = Payment::pluck('status', 'id')->toArray();
        $this->assertEquals($initialStatuses, $postStatuses);
    }

    public function test_37_no_assignment_created_during_reporting(): void
    {
        $initialAssignments = CandidateTestAssignment::count();

        $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $this->actingAs($this->financeUser)->get(route('finance.payments.export.csv'));

        $this->assertEquals($initialAssignments, CandidateTestAssignment::count());
    }

    public function test_38_no_attempt_created_during_reporting(): void
    {
        $initialAttempts = Attempt::count();

        $this->actingAs($this->financeUser)->get(route('finance.payments.index'));

        $this->assertEquals($initialAttempts, Attempt::count());
    }

    public function test_39_no_certificate_created_during_reporting(): void
    {
        $initialCerts = Certificate::count();

        $this->actingAs($this->financeUser)->get(route('finance.payments.index'));

        $this->assertEquals($initialCerts, Certificate::count());
    }

    public function test_40_finance_product_voucher_restrictions_remain_intact(): void
    {
        $resProduct = $this->actingAs($this->financeUser)->post(route('admin.commerce.products.store'), [
            'title' => 'Illegal Product',
        ]);
        $resProduct->assertStatus(403);

        $resVoucher = $this->actingAs($this->financeUser)->post(route('admin.commerce.vouchers.store'), [
            'code' => 'ILLEGAL50',
        ]);
        $resVoucher->assertStatus(403);
    }

    public function test_41_ra_commerce_remains_intact(): void
    {
        $response = $this->actingAs($this->adminUser)->get(route('admin.commerce.index'));
        $response->assertStatus(200);
    }

    public function test_42_sa_price_approval_remains_intact(): void
    {
        $response = $this->actingAs($this->superAdminUser)->get(route('admin.approvals.index'));
        $response->assertStatus(200);
    }

    public function test_43_light_theme_contract_passes(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $response->assertStatus(200);
        $response->assertSee('text-slate-900', false);
        $response->assertSee('bg-white', false);
    }

    public function test_44_dark_theme_contract_passes(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $response->assertStatus(200);
        $response->assertSee('dark:bg-slate-900', false);
        $response->assertSee('dark:text-white', false);
    }

    public function test_45_global_theme_contract_remains_intact(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.dashboard'));
        $response->assertStatus(200);
        $response->assertSee('Gross Cash Collections');
        $response->assertSee('Unpaid Invoices');
        $response->assertSee('Pending Approvals');
    }

    public function test_46_finance_reports_render_start_and_end_date_empty_with_placeholder_by_default(): void
    {
        $response = $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $response->assertStatus(200);
        $response->assertSee('Select day/mo/year');
        $response->assertSee('name="start_date" id="start_date" x-model="val" value=""', false);
        $response->assertSee('name="end_date" id="end_date" x-model="val" value=""', false);
    }

    public function test_47_clear_filters_removes_date_query_parameters_and_empties_fields(): void
    {
        // When filtered with dates
        $responseFiltered = $this->actingAs($this->financeUser)->get(route('finance.payments.index', [
            'start_date' => '2026-08-20',
            'end_date'   => '2026-08-27',
        ]));
        $responseFiltered->assertStatus(200);
        $responseFiltered->assertSee('value="2026-08-20"', false);
        $responseFiltered->assertSee('value="2026-08-27"', false);

        // When Clear is clicked (navigates to base index)
        $responseCleared = $this->actingAs($this->financeUser)->get(route('finance.payments.index'));
        $responseCleared->assertStatus(200);
        $responseCleared->assertSee('Select day/mo/year');
        $responseCleared->assertSee('name="start_date" id="start_date" x-model="val" value=""', false);
        $responseCleared->assertSee('name="end_date" id="end_date" x-model="val" value=""', false);
        $responseCleared->assertSee('PAY-20260827-VZDM');
        $responseCleared->assertSee('PAY-20260820-CONF');
        $responseCleared->assertSee('PAY-20260815-FAIL');
    }
}
