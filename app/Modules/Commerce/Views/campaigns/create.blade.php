...
<div class="max-w-4xl mx-auto space-y-6" x-data="{
    ...
    <div class="flex items-center gap-2 text-xs text-slate-400">
        ...
        <span>/</span>
        <span class="text-white font-medium">Create Voucher Campaign</span>
    </div>

    <!-- Header -->
    <div class="flex items-start justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
            <h1 class="text-2xl font-bold text-white">Generate Voucher Campaign</h1>
            <p class="text-xs text-slate-400 mt-1">Configure an assessment-family governed promotional campaign and generate unique secure voucher codes.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.commerce.campaigns.store') }}" class="space-y-6">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
            <h2 class="text-sm font-bold text-white uppercase tracking-wider text-indigo-400">1. Campaign Identity &amp; Assessment Family</h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" x-model="name" required placeholder="e.g. TOEIC Institutional UAT Promotion"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-950 text-white text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>

                <div>
                    <label for="assessment_family" class="block text-xs font-semibold text-slate-300 mb-1.5">
                    ...
                    <select id="assessment_family" name="assessment_family" x-model="family" required
                            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-950 text-white text-xs font-semibold focus:ring-2 focus:ring-indigo-500 focus:outline-none">
            ...
            <div class="pt-2 space-y-3">
                <label class="block text-xs font-semibold text-slate-300">Product Applicability Scope <span class="text-rose-500">*</span></label>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label class="flex items-start gap-3 p-3.5 rounded-xl border cursor-pointer transition"
                           :class="scopeMode === 'all_products_in_family' ? 'border-indigo-500 bg-indigo-950/40 text-white' : 'border-slate-800 bg-slate-950 text-slate-400 hover:border-slate-700'">
                        ...
                        <div>
                            ...
                            <span class="text-[11px] text-slate-400 block mt-0.5">Applies to all active packages belonging to the selected family.</span>
                        </div>
                    </label>

                    <label class="flex items-start gap-3 p-3.5 rounded-xl border cursor-pointer transition"
                           :class="scopeMode === 'selected_products' ? 'border-indigo-500 bg-indigo-950/40 text-white' : 'border-slate-800 bg-slate-950 text-slate-400 hover:border-slate-700'">
                        ...
                        <div>
                            ...
                            <span class="text-[11px] text-slate-400 block mt-0.5">Explicitly choose which packages are eligible for this campaign.</span>
                        </div>
                    </label>
                </div>
            </div>
            ...
            <div x-show="scopeMode === 'selected_products'" x-transition class="pt-2 space-y-2 border-t border-slate-800/80">
                <label class="block text-xs font-semibold text-slate-300">Choose Eligible Packages (Filtered by Family):</label>
                <div class="space-y-2 max-h-48 overflow-y-auto p-3 rounded-xl bg-slate-950 border border-slate-800">
                    <template x-if="filteredProducts.length === 0">
                        <p class="text-xs text-amber-400 p-2">No active commercial products found for <span x-text="family.toUpperCase()"></span> family.</p>
                    </template>
                    <template x-for="prod in filteredProducts" :key="prod.id">
                        <label class="flex items-center justify-between p-2 rounded-lg hover:bg-slate-900 cursor-pointer text-xs">
                            <div class="flex items-center gap-2">
                                <input type="checkbox" name="products[]" :value="prod.id" x-model="selectedProductIds" class="rounded border-slate-700 text-indigo-600 focus:ring-indigo-500">
                                <span class="font-medium text-slate-200" x-text="prod.title"></span>
                            </div>
                            <span class="text-slate-400 font-mono" x-text="'IDR ' + new Intl.NumberFormat('id-ID').format(prod.price)"></span>
                        </label>
                    </template>
                </div>
            </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
            <h2 class="text-sm font-bold text-white uppercase tracking-wider text-indigo-400">2. Discount Policy &amp; Validity Window</h2>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="discount_type" class="block text-xs font-semibold text-slate-300 mb-1.5">Discount Type</label>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" id="discount_value" name="discount_value" x-model.number="discountValue" min="1" :max="discountType === 'percentage' ? 100 : 100000000" required
                               class="w-full px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-950 text-white text-xs font-bold focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        <span class="absolute right-3.5 top-2.5 text-xs text-slate-400 font-bold" x-text="discountType === 'percentage' ? '%' : 'IDR'"></span>
                    </div>
                </div>

                <div>
                    <label for="valid_from" class="block text-xs font-semibold text-slate-300 mb-1.5">Valid From (Optional)</label>
                    <input type="datetime-local" id="valid_from" name="valid_from" value="{{ old('valid_from', now()->format('Y-m-d\TH:i')) }}"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-950 text-white text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>

                <div>
                    <label for="valid_until" class="block text-xs font-semibold text-slate-300 mb-1.5">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
            <h2 class="text-sm font-bold text-white uppercase tracking-wider text-indigo-400">3. Secure Code Generation &amp; Capacity</h2>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-semibold text-slate-300 mb-1.5">Generation Mode</label>
                    <div class="grid grid-cols-2 gap-2">
                        <label class="flex items-center justify-center p-2.5 rounded-xl border cursor-pointer text-xs font-bold transition"
                               :class="generationMode === 'shared' ? 'border-indigo-500 bg-indigo-950/50 text-white' : 'border-slate-800 bg-slate-950 text-slate-400'">
                            <input type="radio" name="generation_mode" value="shared" x-model="generationMode" class="sr-only">
                            <span>Shared Single Code</span>
                        </label>
                        <label class="flex items-center justify-center p-2.5 rounded-xl border cursor-pointer text-xs font-bold transition"
                               :class="generationMode === 'batch' ? 'border-indigo-500 bg-indigo-950/50 text-white' : 'border-slate-800 bg-slate-950 text-slate-400'">
                ...
                <div x-show="generationMode === 'batch'">
                    <label for="total_codes" class="block text-xs font-semibold text-slate-300 mb-1.5">Number of Unique Codes (1–500)</label>
                    <input type="number" id="total_codes" name="total_codes" x-model.number="totalCodes" min="1" max="500"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-950 text-white text-xs font-semibold focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>

                <div>
                    <label for="uses_per_code" class="block text-xs font-semibold text-slate-300 mb-1.5">
                ...
                <div>
                    <label for="code_prefix" class="block text-xs font-semibold text-slate-300 mb-1.5">Code Prefix</label>
                    <input type="text" id="code_prefix" name="code_prefix" x-model="customPrefix" :placeholder="defaultPrefix" maxlength="20"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-950 text-white text-xs font-mono uppercase focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>

                <div>
                    <label for="code_length" class="block text-xs font-semibold text-slate-300 mb-1.5">Random Suffix Length (4–12)</label>
            ...
            <div class="p-4 rounded-xl bg-slate-950 border border-slate-800/80 flex items-center justify-between">
                <div>
                    <span class="text-[10px] uppercase tracking-wider font-bold text-slate-400 block">Generated Code Format Sample</span>
                    <span class="font-mono text-base font-bold text-emerald-400 block mt-0.5" x-text="previewCode"></span>
                    <span class="text-[11px] text-slate-500">Omits ambiguous characters (0, O, 1, I, L) for maximum clarity.</span>
                </div>
                <div class="text-right">
                    <span class="text-xs font-bold text-slate-300 block" x-text="generationMode === 'shared' ? '1 Code (' + usesPerCode + ' uses)' : totalCodes + ' Codes (' + usesPerCode + ' use/each)'"></span>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-800">
            <a href="{{ route('admin.commerce.index') }}" class="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold transition">
                Cancel
            </a>
