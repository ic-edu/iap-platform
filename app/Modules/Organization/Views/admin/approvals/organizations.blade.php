...
@section('content')
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <span class="px-3 py-1 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-full text-xs font-semibold">
                {{ $pendingOrganizations->count() }} Pending Organizations
            </span>
            @if($pendingGroups->count() > 0)
                <span class="px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 rounded-full text-xs font-semibold">
    ...
    <div class="mb-8 bg-slate-950/80 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div class="p-4 bg-slate-900/60 border-b border-slate-800 flex items-center justify-between">
            ...
            <span class="text-xs text-slate-400">{{ $pendingOrganizations->count() }} waiting for review</span>
        </div>

        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="border-b border-slate-800 bg-slate-900/30 text-[11px] font-semibold uppercase text-slate-400">
                ...
                <tbody class="divide-y divide-slate-800/60">
                    @forelse($pendingOrganizations as $org)
                        <tr class="hover:bg-slate-900/40 transition-colors">
                            ...
                            <td class="py-3 px-4 text-slate-400 capitalize">{{ $org->organization_type->label() }}</td>
                            ...
                            <td class="py-3 px-4 text-right space-x-2 whitespace-nowrap">
                                ...
                                <div id="revision-modal-{{ $org->id }}" class="hidden fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 text-left">
                                    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full shadow-2xl">
                                        <h3 class="text-sm font-bold text-white mb-1">Return {{ $org->name }} for Revision</h3>
                                        <p class="text-xs text-slate-400 mb-4">Provide clear instructions to Registration Admin on what information or documentation is needed.</p>
                                        <form method="POST" action="{{ route('admin.approvals.organizations.return-revision', $org->id) }}">
                                            @csrf
                                            <div class="mb-4">
                                                <label class="block text-xs font-medium text-slate-300 mb-1">Revision Note *</label>
                                                <textarea name="revision_note" rows="3" required placeholder="e.g. Please clarify campus address and official institutional domain..." class="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-white focus:border-amber-500 focus:outline-none"></textarea>
                                            </div>
                                            <div class="flex justify-end gap-2">
                                                <button type="button" onclick="document.getElementById('revision-modal-{{ $org->id }}').classList.add('hidden')" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs rounded-lg">Cancel</button>
                                ...
                                <div id="reject-modal-{{ $org->id }}" class="hidden fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 text-left">
                                    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full shadow-2xl">
                                        <h3 class="text-sm font-bold text-white mb-1">Reject {{ $org->name }}</h3>
                                        <p class="text-xs text-slate-400 mb-4">Specify the governance or policy reason for rejecting this organization.</p>
                                        <form method="POST" action="{{ route('admin.approvals.organizations.reject', $org->id) }}">
                                            @csrf
                                            <div class="mb-4">
                                                <label class="block text-xs font-medium text-slate-300 mb-1">Rejection Reason *</label>
                                                <textarea name="rejection_reason" rows="3" required placeholder="e.g. Institution does not meet platform accreditation criteria..." class="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-white focus:border-rose-500 focus:outline-none"></textarea>
                                            </div>
                                            <div class="flex justify-end gap-2">
                                                <button type="button" onclick="document.getElementById('reject-modal-{{ $org->id }}').classList.add('hidden')" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs rounded-lg">Cancel</button>
    ...
    @if($pendingGroups->count() > 0)
        <div class="mb-8 bg-slate-950/80 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
            <div class="p-4 bg-slate-900/60 border-b border-slate-800 flex items-center justify-between">
                ...
                <span class="text-xs text-slate-400">{{ $pendingGroups->count() }} awaiting approval</span>
            </div>
                ...
                <table class="w-full text-left border-collapse text-xs">
                    ...
                    <tbody class="divide-y divide-slate-800/60">
                        @foreach($pendingGroups as $grp)
                            <tr class="hover:bg-slate-900/40 transition-colors">
                                <td class="py-3 px-4 font-bold text-white">{{ $grp->name }}</td>
                                <td class="py-3 px-4 text-slate-300">{{ $grp->organization?->name }}</td>
                                <td class="py-3 px-4 text-slate-400 capitalize">{{ $grp->group_type ?? 'Standard' }}</td>
                                <td class="py-3 px-4 text-slate-400">{{ $grp->submitter?->name ?? 'Registration Admin' }}</td>
    ...
    <div class="bg-slate-950/80 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div class="p-4 bg-slate-900/60 border-b border-slate-800 flex items-center justify-between">
            ...
            <span class="text-xs text-slate-400">{{ $activeOrganizations->total() }} total</span>
        </div>
                ...
                <tbody class="divide-y divide-slate-800/60">
                    @forelse($activeOrganizations as $org)
                        <tr class="hover:bg-slate-900/40 transition-colors">
                            ...
                            <td class="py-3 px-4 text-slate-400 capitalize">{{ $org->organization_type->label() }}</td>
                            <td class="py-3 px-4">
                                <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase {{ $org->isActive() ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-slate-500/20 text-slate-400 border border-slate-500/30' }}">
                                    {{ $org->status->label() }}
                                </span>
                            </td>
                            <td class="py-3 px-4 text-slate-400">
                            ...
                            <td class="py-3 px-4 text-right space-x-2">
                                ...
                                @if(!$org->isArchived())
                                    <form method="POST" action="{{ route('admin.approvals.organizations.archive', $org->id) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="text-xs font-medium text-slate-400 hover:text-rose-400">Archive</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
