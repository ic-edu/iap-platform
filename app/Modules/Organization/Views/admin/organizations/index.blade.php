                        ...
                        <tr class="hover:bg-slate-900/40 transition-colors">
                            <td class="py-3 px-4">
                                ...
                                @if($org->needsRevision() && $org->revision_note)
                                    <div class="mt-1.5 p-2 bg-amber-500/10 border border-amber-500/20 rounded text-[11px] text-amber-400">
                                        <span class="font-semibold">Revision Note:</span> {{ $org->revision_note }}
                                    </div>
                                @elseif($org->isRejected() && $org->rejection_reason)
                                    <div class="mt-1.5 p-2 bg-rose-500/10 border border-rose-500/20 rounded text-[11px] text-rose-400">
                            ...
                            <td class="py-3 px-4">
                                @if($org->isActive())
                                    <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                                        Active
                                    </span>
                                @elseif($org->isPending())
                                    <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-500/20 text-amber-400 border border-amber-500/30">
                                        Pending Approval
                                    </span>
                                @elseif($org->needsRevision())
                                    <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase bg-orange-500/20 text-orange-400 border border-orange-500/30">
                                        Needs Revision
                                    </span>
                                @elseif($org->isRejected())
                                    <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase bg-rose-500/20 text-rose-400 border border-rose-500/30">
                                        Rejected
                                    </span>
                                @elseif($org->isSuspended())
                                    <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase bg-slate-500/20 text-slate-400 border border-slate-500/30">
                                        Suspended
                                    </span>
                                @elseif($org->isArchived())
                                    <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase bg-zinc-500/20 text-zinc-400 border border-zinc-500/30">
                                        Archived
                                    </span>
                                @endif
                            </td>
                            <td class="py-3 px-4 text-right space-x-2 whitespace-nowrap">
                                @if($org->isActive())
                                    ...
                                    <button type="button" onclick="document.getElementById('invite-modal-{{ $org->id }}').classList.remove('hidden')" class="text-xs font-semibold text-teal-400 hover:text-teal-300">
                                        + Invite Coord
                                    </button>
                                    ...
                                    <div id="invite-modal-{{ $org->id }}" class="hidden fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 text-left">
                                        <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full shadow-2xl">
                                            <h3 class="text-sm font-bold text-white mb-1">Invite Primary Coordinator</h3>
                                            <p class="text-xs text-slate-400 mb-4">Send an invitation to the primary coordinator for <strong class="text-white">{{ $org->name }}</strong>.</p>
                                            <form method="POST" action="{{ route('admin.organizations.invite-coordinator', $org->id) }}">
                                                @csrf
                                                <div class="mb-4">
                                                    <label class="block text-xs font-medium text-slate-300 mb-1">Coordinator Email *</label>
                                                    <input type="email" name="coordinator_email" required placeholder="coordinator@example.com" class="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-white focus:border-indigo-500 focus:outline-none">
                                                </div>
                                                <div class="flex justify-end gap-2">
                                                    <button type="button" onclick="document.getElementById('invite-modal-{{ $org->id }}').classList.add('hidden')" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs rounded-lg">Cancel</button>
                                ...
                                @elseif($org->isPending())
                                    <a href="{{ route('admin.organizations.edit', $org->id) }}" class="text-xs font-semibold text-slate-400 hover:text-slate-300">Edit</a>
                                    <span class="text-[11px] text-amber-400 italic">Awaiting Approval</span>
                                @elseif($org->isSuspended())
                                    <a href="{{ route('admin.organizations.edit', $org->id) }}" class="text-xs font-semibold text-slate-400 hover:text-slate-300">Edit</a>
                                    <form method="POST" action="{{ route('admin.organizations.activate', $org->id) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="text-xs font-medium text-emerald-400 hover:text-emerald-300">
                                            Reactivate
                                        </button>
                                    </form>
                                @elseif($org->isRejected())
                                    <a href="{{ route('admin.organizations.edit', $org->id) }}" class="text-xs font-semibold text-slate-400 hover:text-slate-300">Edit & Reapply</a>
                                @endif
                            </td>
                        </tr>
                    @empty
