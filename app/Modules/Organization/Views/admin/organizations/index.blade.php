                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                            ...
                            <td class="py-3 px-4">
                                @if($org->isActive())
                                    ...
                                    @if($org->hasPendingSuspension())
                                        <div class="mt-1">
                                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-500/20 text-amber-700 dark:text-amber-400 border border-amber-500/30">
                                                ⏳ Suspension Pending
                                            </span>
                                        </div>
                                    @endif
                            ...
                            <td class="py-3 px-4 text-right space-x-2 whitespace-nowrap">
                                @if($org->isActive())
                                    ...
                                    <!-- Governed Suspension Request Trigger -->
                                    @if($org->hasPendingSuspension())
                                        <span class="inline-flex items-center gap-1 text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-500/10 border border-amber-500/30 px-2 py-1 rounded">
                                            ⏳ Suspension Pending
                                        </span>
                                    @else
                                        <button type="button" onclick="document.getElementById('suspend-request-modal-{{ $org->id }}').classList.remove('hidden')" class="text-xs font-semibold text-amber-600 hover:text-amber-500 dark:text-amber-400 dark:hover:text-amber-300">
                                            Request Suspension
                                        </button>
                                    @endif

                                    <!-- Suspension Request Modal (Governed Transition) -->
                                    <div id="suspend-request-modal-{{ $org->id }}" class="hidden fixed inset-0 z-50 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 text-left whitespace-normal">
                                            ...
                                            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-slate-800">
                                                <div>
                                                    <h3 class="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                                                        ...
                                                        <span>Request Organization Suspension</span>
                                                    </h3>
                                                    <p class="text-[11px] text-slate-500 dark:text-slate-400">{{ $org->name }} ({{ $org->organization_type->label() }})</p>
                                                </div>
                                                <button type="button" onclick="document.getElementById('suspend-request-modal-{{ $org->id }}').classList.add('hidden')" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 text-sm">✕</button>
                                            </div>
                                            <p class="text-xs text-slate-600 dark:text-slate-400 mb-4 leading-relaxed">
                                                Organization suspension is a governance-level state transition requiring Super Admin review and approval. Once submitted, the organization will remain <strong class="text-slate-900 dark:text-white">Active</strong> until approved.
                                            </p>
                                            <form method="POST" action="{{ route('admin.organizations.request-suspension', $org->id) }}">
                                                @csrf
                                                <div class="mb-4">
                                                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Suspension Reason *</label>
                                                    <input type="text" name="reason" required placeholder="e.g., Contract non-compliance, security investigation" class="w-full p-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs text-slate-900 dark:text-white focus:border-amber-500 focus:outline-none">
                                                </div>
                                                <div class="mb-4">
                                                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Additional Operational Notes (Optional)</label>
                                                    <textarea name="notes" rows="3" placeholder="Provide background context, reference ticket IDs, or timelines..." class="w-full p-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs text-slate-900 dark:text-white focus:border-amber-500 focus:outline-none"></textarea>
                                                </div>
                                                <div class="flex justify-end gap-2">
                                                    <button type="button" onclick="document.getElementById('suspend-request-modal-{{ $org->id }}').classList.add('hidden')" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-lg">Cancel</button>
                                                    <button type="submit" class="px-4 py-1.5 bg-amber-600 hover:bg-amber-500 text-white text-xs font-semibold rounded-lg shadow">Submit Request</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
