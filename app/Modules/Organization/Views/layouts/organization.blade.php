            ...
            <div class="p-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false">
                    <button type="button"
                            ...
                            id="org-user-menu-btn"
                            class="w-full flex items-center justify-between p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/80 transition text-left cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-indigo-500/40">
                        <div class="flex items-center gap-2.5 overflow-hidden">
                            <div class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 flex items-center justify-center text-xs font-bold shrink-0">
                                {{ strtoupper(substr(Auth::user()->name ?? 'U', 0, 1)) }}
                            </div>
                            <div class="truncate text-xs font-semibold text-slate-800 dark:text-slate-200">
                                {{ Auth::user()->name ?? 'User' }}
                            </div>
                        </div>
                    ...
                    <div x-show="open"
                         ...
                         id="org-user-menu-dropdown"
                         class="absolute bottom-full left-0 right-0 mb-2 p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 text-xs space-y-2"
                         style="display: none;"
                         role="menu"
                         aria-labelledby="org-user-menu-btn">

                        <!-- User Info Header -->
                        <div class="px-2 py-1.5 border-b border-slate-100 dark:border-slate-800">
                            <p class="font-semibold text-slate-900 dark:text-white truncate">{{ Auth::user()->name ?? 'User' }}</p>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 font-mono truncate">{{ Auth::user()->email ?? '' }}</p>
                        </div>

                        <!-- Appearance Submenu / Controls -->
                        <div class="p-2 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-100 dark:border-slate-800/80 space-y-1.5">
                            ...
                            <div class="grid grid-cols-3 gap-1 p-0.5 bg-slate-200/60 dark:bg-slate-900 border border-slate-300/60 dark:border-slate-800 rounded-lg" role="group" aria-label="Theme selector">
                                <button type="button"
                                        ...
                                        id="theme-btn-light"
                                        class="theme-switcher-btn flex items-center justify-center p-1.5 rounded-md text-xs font-bold transition-all cursor-pointer focus:outline-hidden"
                                        title="Light mode"
                                ...
                                <button type="button"
                                    ...
                                    <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
                                </button>
                                <button type="button"
                                    ...
                                    <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                </button>
                            </div>
                        </div>
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="block">
                            @csrf
                            <button type="submit" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-xl transition cursor-pointer">
