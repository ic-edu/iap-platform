<x-admin-layout>
    ...
    <div class="grid gap-6 lg:grid-cols-3 mb-8">
        <!-- New Test Builder Form (Teacher & Admin Only) -->
                            ...
                            <tr>
                                ...
                                <td class="p-3">
                                    @if ($test->is_published || $test->status === 'published')
                                        <span class="px-2.5 py-1 text-xs font-bold rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase">Published Live</span>
                                    @elseif ($test->status === 'approved')
                                        <span class="px-2.5 py-1 text-xs font-bold rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase">Approved by SA</span>
                                    @elseif ($test->status === 'pending_approval')
                                        <span class="px-2.5 py-1 text-xs font-bold rounded bg-amber-500/10 text-amber-400 border border-amber-500/20 uppercase">Pending Approval</span>
                                    @elseif ($test->status === 'rejected')
                                        <span class="px-2.5 py-1 text-xs font-bold rounded bg-rose-500/10 text-rose-400 border border-rose-500/20 uppercase">Rejected</span>
                                    @else
                                        <span class="px-2.5 py-1 text-xs font-bold rounded bg-slate-800 text-slate-400 border border-slate-700 uppercase">Draft</span>
                                    @endif
                                </td>
                                <td class="p-3 text-right space-x-2">
                                    @if (!Auth::user()?->hasRole('super-admin') && ($test->status === 'draft' || $test->status === 'rejected' || empty($test->status)))
                                    ...
                                    @if (Auth::user()?->hasRole('admin') && $test->status === 'approved')
                                        <form method="POST" action="{{ route('admin.tests.publish', $test->id) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs text-emerald-400 font-bold hover:underline">Publish Approved Test</button>
                                        </form>
                                    @endif
