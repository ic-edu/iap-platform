<x-admin-layout>
    ...
    <div class="grid gap-6 lg:grid-cols-3 mb-8">
        ...
        <div class="lg:col-span-2 bg-slate-950 border border-slate-800 rounded-xl p-6 shadow-sm">
            <h3 class="text-base font-semibold text-white mb-4">{{ __('Tests Listing') }}</h3>
                    ...
                    <tbody class="divide-y divide-slate-800/60">
                        @forelse ($tests as $test)
                            <tr>
                                ...
                                <td class="p-3">{{ $test->duration_minutes }} Mins</td>
                                <td class="p-3">
                                    @if ($test->is_published)
                                        <span class="px-2 py-0.5 text-xs font-medium rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">Published</span>
                                    @else
                                        <span class="px-2 py-0.5 text-xs font-medium rounded bg-amber-500/10 text-amber-400 border border-amber-500/20">Draft</span>
                                    @endif
                                </td>
                                <td class="p-3 text-right space-x-2">
                                    @if (!$test->is_published)
                                        <form method="POST" action="{{ route('admin.tests.publish', $test) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs text-emerald-400 hover:underline">Publish</button>
                                        </form>
                                    @endif
